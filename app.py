    ...
    if not inv_file or not pl_file or not nomor_aju:
        ...
        with st.spinner("Memproses data..."):
            try:
                ...
                # --- SHEET HEADER ---
                df_header = pd.DataFrame(columns=['NOMOR AJU', 'KODE DOKUMEN', 'KODE KANTOR', 'KODE JENIS IMPOR', 'KODE JENIS PROSEDUR', 'KODE CARA BAYAR', 'TANGGAL PERNYATAAN', 'KOTA PERNYATAAN', 'KODE ASURANSI', 'KODE PELABUHAN TUJUAN', 'NAMA PERNYATAAN', 'JABATAN PERNYATAAN', 'FLAG PROPORSIONAL NETTO', 'ASURANSI', 'NILAI BARANG', 'BIAYA TAMBAHAN', 'BIAYA PENGURANG', 'VD', 'HARGA_PENYERAHAN', 'DASAR PENGENAAN PAJAK', 'UANG MUKA', 'VOLUME', 'PPN PAJAK', 'NETTO', 'BRUTO', 'NDPBM', 'FOB', 'CIF'])
                ...
                # --- SHEET ENTITAS (Logika Sesuai Permintaan Anda) ---
                entitas_cols = ['NOMOR AJU', 'SERI', 'KODE ENTITAS', 'KODE JENIS IDENTITAS', 'NOMOR IDENTITAS', 'NAMA ENTITAS', 'ALAMAT ENTITAS', 'NIB ENTITAS', 'KODE JENIS API', 'KODE STATUS', 'NOMOR IJIN ENTITAS', 'TANGGAL IJIN ENTITAS', 'KODE NEGARA', 'NIPER ENTITAS', 'KODE KATEGORI KONSOLIDATOR', 'KODE AFILIASI']
                df_entitas = pd.DataFrame(columns=entitas_cols)
                
                # Data untuk 7 baris
                data_entitas = {
                    'NOMOR AJU': [nomor_aju] * 7,
                    'SERI': [10, 9, 11, 1, 7, 4, 10], # Baris 7 diisi 10 sebagai contoh
                    'KODE ENTITAS': [10, 9, 11, 1, 7, 4, 10],
                    'KODE JENIS IDENTITAS': [None, None, None, 6, 6, 6, 6] # Kolom identitas diisi 6 dari baris 4 ke 7
                }
