...
# --- 4. AREA UPLOAD (Dibuat grid/kolom agar rapi) ---
...
# --- 5. DAFTAR KOLOM CEISA (Sesuai Template Asli) ---
ceisa_columns = [
    ...
    Penyebab utama dari *error* pada *script* yang Anda berikan adalah adanya karakter spasi yang tidak kasat mata (*non-breaking spaces* atau `\xa0`) di awal baris kode. Hal ini sangat sering terjadi ketika kode disalin (*copy-paste*) dari sumber tertentu, yang menyebabkan Python tidak bisa membaca indentasi dengan benar (`IndentationError` atau `SyntaxError`).

Saya telah membersihkan seluruh *script* tersebut, memperbaiki indentasinya menggunakan standar Python (4 spasi), dan memastikan strukturnya siap digunakan. 

Karena Anda tidak menyertakan "script yang baru" di dalam pesan Anda sebelumnya, berikut adalah **versi script lama Anda yang sudah diperbaiki total** agar bisa berjalan tanpa error. Jika Anda memiliki bagian *script* baru yang ingin digabungkan ke dalamnya (misalnya logika tambahan untuk Packing List atau B/L), Anda bisa memberikannya setelah ini.
